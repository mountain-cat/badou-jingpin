...
bert = BertModel.from_pretrained(r"C:\Users\18344\Desktop\wxw\bert_chinese", return_dict=False)
...
#######下面是新添内容，使用gptj结构########################
#######################################################
    def all_gptj_forward(self, x):
        for i in range(self.num_layers):
            x = self.single_gptj_forward(x, i)
        return x
    def single_gptj_forward(self, x, index):
        weights = self.transformer_weights[index]
        # 取出该层的参数，在实际中，这些参数都是随机初始化，之后进行预训练
        q_w, q_b, \
            k_w, k_b, \
            v_w, v_b, \
        ...
        feed_forward_x = self.feed_forward(x,
                                           intermediate_weight, intermediate_bias,
                                           output_weight, output_bias)
        x = self.layer_norm(x + feed_forward_x + attention_output, ff_layer_norm_w, ff_layer_norm_b)

        return x
    def gptj_forward(self,x):
        x = self.embedding_forward(x)
        sequence_output = self.all_gptj_forward(x)
        pooler_output = self.pooler_output_layer(sequence_output[0])
        return sequence_output, pooler_output
...
diy_gptjs_output, diy_gptjp_output = db.gptj_forward(x)
#torch
