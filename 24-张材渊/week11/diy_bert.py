...
bert = BertModel.from_pretrained(r"C:\Users\zhang\Desktop\week作业\badou-jingpin\24-张材渊\week6\bert-base-chinese",
                                 return_dict=False)
...
class DiyBert:
    ...
    def single_transformer_layer_forward(self, x, layer_index):
        ...
        # self attention层
        residual = x
        hidden_states = self.layer_norm(x, attention_layer_norm_w, attention_layer_norm_b)
        ...
        # feed forward层
        feed_forward_x = self.feed_forward(hidden_states,
                                           intermediate_weight, intermediate_bias,
                                           output_weight, output_bias)
        # # bn层，并使用了残差机制
        # x = self.layer_norm(x + feed_forward_x, ff_layer_norm_w, ff_layer_norm_b)
        hidden_states = attention_output + feed_forward_x + residual
