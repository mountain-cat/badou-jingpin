...
class SiameseNetwork(nn.Module):
    ...
    # sentence : (batch_size, max_length)，sentence1, sentence2是相似样本，sentence3是不相似样本（此处有修改）
    def forward(self, sentence1, sentence2=None, sentence3=None):
        ...
        if sentence2 is not None:
            ...
            # 如果有标签，则计算triplet loss
            if sentence3 is not None:
                vector3 = self.sentence_encoder(sentence3)
                return self.cosine_triplet_loss(vector1, vector2, vector3, margin=0.5)
            # 如果无标签，计算余弦距离
            else:
