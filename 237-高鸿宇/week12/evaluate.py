...
class Evaluator:
    def __init__(self, opt, model, logger):
        ...
        self.model = model
        self.logger = logger
        self.valid_data = load_data(opt, shuffle=False)
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            sentences = self.valid_data.dataset.sentences[index * self.opt.batch_size: (index+1) * self.opt.batch_size]
            if torch.cuda.is_available():
                batch_data = [d.cuda() for d in batch_data]
            input_id, labels = batch_data   #输入变化时这里需要修改，比如多输入，多输出的情况
            with torch.no_grad():
                pred_results = self.model(input_id) #不输入labels，使用模型当前参数进行预测
                pred_results = pred_results.logits
