...
通过手动矩阵运算实现Bert结构, 基于GPTJ结构

'''

bert = BertModel.from_pretrained("bert", return_dict=False)
...
class DiyBert:
    ...
    def single_transformer_layer_forward(self, x, layer_index):
        ...
        attention_output = self.self_attention(self.layer_norm(x, ff_layer_norm_w, ff_layer_norm_b),
                                                q_w, q_b,
                                                k_w, k_b,
                                                v_w, v_b,
                                                attention_output_weight, attention_output_bias,
        ...
        feed_forward_x = self.feed_forward(self.layer_norm(x, ff_layer_norm_w, ff_layer_norm_b),
                                  intermediate_weight, intermediate_bias,
                                  output_weight, output_bias)
        return x + attention_output + feed_forward_x
