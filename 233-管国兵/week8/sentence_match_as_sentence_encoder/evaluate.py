...
class Evaluator:
    ...
    def write_stats(self, test_question_vectors, labels):
        ...
        for test_question_vector, label in zip(test_question_vectors, labels):
            ...
            # 计算一个向量和一个矩阵的相似度，可以把向量转为归一化后的结果，把矩阵里的每个向量也做归一化，然后做点积。因为两个向量的cosine = 两个归一化后的向量做乘积
            res = torch.mm(test_question_vector.unsqueeze(0),
                           self.knwb_vectors.T)  # 这里直接做了点乘，输入的向量已经在前面代码里做了归一化了：line 36
