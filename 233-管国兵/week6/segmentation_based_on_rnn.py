...
"""
中文分词加入bert（https://huggingface.co/bert-base-chinese）
config.json : 降低配置后bert的config
vocab.txt : bert的字表
...
def main():
    ...
    for epoch in range(epoch_num):
        ...
        for x, y in data_loader:
            ...
            print('第', epoch, '轮, 第', i, '次，loss = ', loss)
            i += 1
            loss.backward()  # 计算梯度
            optim.step()  # 更新权重
...
第 0 轮, 第 144 次，loss =  tensor(0.4304, grad_fn=<NllLossBackward0>)
    ...
    input_strings = ["同时国内有望出台新汽车刺激方案",
                     ...
                     "经过两个交易日的强势调整后",
                     "昨日上海天然橡胶期货价格再度大幅上扬"]
    predict("model.pth", "vocab.txt", input_strings)
