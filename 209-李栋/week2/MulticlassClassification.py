...
def evaluate(model):
    ...
    y1 = [a for a in y if a == 1]
    y2 = [b for b in y if b == 2]
    # print("本次预测集中共有%d个正样本，%d个负样本" % (sum(y), test_sample_num - sum(y)))
    print("本次预测集中共有%d个样本一，%d个样本二，%d个样本三" % (len(y1), len(y2), test_sample_num - len(y1) - len(y2)))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        ...
        print("输入：%s, 预测类别：%d, 概率值：%.2f" % (vec, torch.argmax(res), max(res)))  # 打印结果
